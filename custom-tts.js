...
export class CustomTTSStream extends tts.ChunkedStream {
    ...
    async convertAudioToFrame(audioData) {
        try {
            // Parse WAV file to extract PCM data
            const wavData = this.parseWavFile(audioData);
            // Check if resampling is needed
            if (wavData.sampleRate === this.options.sampleRate &&
                wavData.numChannels === this.options.numChannels) {
                // No resampling needed
                return new AudioFrame(wavData.pcmData, wavData.sampleRate, wavData.numChannels, wavData.samplesPerChannel);
            }
            // Resample if necessary
            const resampler = new AudioResampler(wavData.sampleRate, this.options.sampleRate, wavData.numChannels);
            const inputFrame = new AudioFrame(wavData.pcmData, wavData.sampleRate, wavData.numChannels, wavData.samplesPerChannel);
    ...
    parseWavFile(audioData) {
        ...
        while (offset < audioData.length) {
            const chunkId = new TextDecoder().decode(audioData.slice(offset, offset + 4));
            ...
            if (chunkId === "fmt ") {
                ...
                while (dataOffset < audioData.length) {
                    const dataChunkId = new TextDecoder().decode(audioData.slice(dataOffset, dataOffset + 4));
...
//# sourceMappingURL=custom-tts.js.map